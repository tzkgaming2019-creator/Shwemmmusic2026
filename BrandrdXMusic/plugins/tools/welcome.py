...
async def greet_group(_, member: ChatMemberUpdated):
    ...
    try:
        ...
        temp.MELCOW[f"welcome-{member.chat.id}"] = await app.send_photo(
            ...
            photo=welcomeimg,
            caption=f"""
 ကြိုဆိုပါတယ် {member.chat.title} မှာ
➖➖➖➖➖➖➖➖➖➖➖
๏ နာမည် ➠ {user.mention}
๏ 𝗜𝗗 ➠ {user.id}
๏ အသုံးပြုသူ ➠ @{user.username}
๏ ထုတ်လုပ်သူ ➠ [🇭​𝕒𝕟 🇹​𝕙𝕒𝕣](https://t.me/HANTHAR999)
➖➖➖➖➖➖➖➖➖➖➖
""",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"⦿ ᴀᴅᴅ ᴍᴇ ⦿", url=f"https://t.me/SHWEMM_Music2bot?startgroup=true")]])
        )
    except Exception as e:
        LOGGER.error(e)
    try:
